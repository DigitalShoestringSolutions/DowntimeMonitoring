...
context = zmq.Context()
logger = logging.getLogger("state.event_handler")


@EventHandler.register("downtime/event/+/start")
@EventHandler.register("downtime/event/+/stop")
def handle_message(msg: Event):
    print(f"handling: {msg}")
    try:
        # validate
        timestamp = dateutil.parser.isoparse(msg.content["timestamp"])
        machine_id = msg.content["machine"]
        running = msg.content["running"]
        source = msg.content.get("source", None)

        new_states, updated_states = new_event(machine_id, running, timestamp, source)
...
def _make_mqtt_message(state: State, suffix):
    ...
    return Event(f"{str(state.target.id)}/{suffix}", serializer.data)
